...
"""
    RTScene(graph; message = nothing)

Create a ray tracing scene from a `Graph` object (`graph`) By default, the ray 
tracer will operate on a double floating point precision standard (i.e., 
`Float64`), but it is possible to switch to single precision  by using `Float32` 
as the third argument. This will speed up the computations at the expense of 
numerical accuracy. See VPL documentation for further details.
"""
function RTScene(graph::Graph, ::Type{FT} = Float64; message = nothing) where FT
    # Retrieve a vector of meshes
    mt = MTurtle(FT, message)
    feedgeom!(mt, graph)
    mesh = geoms(mt)

    # Assign to each triangle an id related to the geom it belongs to
    ids_unstr = fill.(1:length(mt.ntriangles), mt.ntriangles)
    ids = reduce(vcat, ids_unstr)
    ...
    rtt = RTTurtle(message = message)
    feedmaterial!(rtt, graph);

    # Create the RTScene
    RTScene(mesh = mesh, ids = ids, materials = materials(rtt))
end

"""
    RTScene(graphs, Float64; message = nothing, parallel = false)

Create a 3D scene for ray tracing from a vector of `Graph` objects (`graphs`). 
...
function RTScene(graphs::Vector{<:Graph}, ::Type{FT} = Float64; 
                 message = nothing, parallel = false) where FT
    scenes = Vector{RTScene}(undef, length(graphs))
    if parallel
        Threads.@threads for i in eachindex(graphs)
            @inbounds scenes[i] = RTScene(graphs[i], FT, message = message)
        end
    else
