...
Create a `Phong` material object from the values of transmittance (`τ`) diffuse reflectance (`ρd`),
maximum Phong specular reflectance (`ρsmax`) and `n` is the specular exponent that controls the "Phong
reflectance lobe". When more than one wavelength is being simulated, a tuple of values should be passed 
for each optical property. See VPL documentation for further details.
"""
...
function generate_ray(material::Phong, ray, disp::Vec{FT}, intersection, interaction, rng) where FT
    ...
    return Ray(intersection.pint .- disp .+ dir.*eps(FT).*2, dir)
end
