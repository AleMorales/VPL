...
"""
Base.@kwdef struct RTSettings{RNG, FT}
    parallel::Bool = false
    pkill::Float64 = 0.2
    maxiter::Int = 2
...
Create a ray tracer object from an acceleration structure built around a 3D mesh (`acceleration`), a grid cloner structure around
the acceleration structure (`grid`), a vector of materials associated to the mesh (`materials`), a vector of sources of irradiance 
(`sources`) and settings. (as generated by `RTSettings()`). See VPL documentation for more details on the ray tracer.
"""
struct RayTracer{A, G, M, S, RT}
    acc::A
    grid::G
    materials::M
    sources::S
...
"""
    RayTracer(scene, sources; settings = RTSettings(), acceleration = Naive, rule = SAH{3}(2,50))
...
"""
function RayTracer(scene::RTScene, sources; settings = RTSettings(), acceleration = Naive, rule = SAH{3}(2,50))
...
"""
    RayTracer(scene, source; settings = RTSettings(), acceleration = Naive, rule = SAH{3}(2,50))

Create a `RayTracer` object from a scene (as generated by `RTScene`), a source (object that inherits from `Source`), 
settings and acceleration function (choose from `Naive` or  `BVH`). The argument `rule` is only required for the 
accelerator `BVH` and it must be an object of type `SAH` or `AvgSplit` (it is ignored for the `Naive` accelerator).
"""
function RayTracer(scene::RTScene, source::Source; settings = RTSettings(), acceleration = Naive, rule = SAH{3}(2,10))
    #acceleration == BVH && println("The BVH acceleration is not working properly yet")
    acc = acceleration(scene.triangles, scene.ids, rule)
    grid = GridCloner(acc; nx = settings.nx, ny = settings.ny, dx = settings.dx, dy = settings.dy)
    RayTracer(acc, grid, scene.materials, (source,), settings)
end
...
function trace!(r::Ray{FT}, rt::RayTracer, materials, tnodestack::Vector{Int}, tdstack::Vector{FT}, 
    ...
    while true
        ...
        hit, intersection, disp = intersect(r, rt.grid, rt.acc, tnodestack, tdstack, nodestack, dstack)
        ...
        # Increase iteration counter (unless it is a Sensor)
        if !isa(material, Sensor)
            iteration += 1
        end
    end
    return iteration
